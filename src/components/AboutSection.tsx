    ...
    <section ref={ref} id="about" className="py-24 md:py-32 px-6 relative overflow-hidden bg-light">
      ...
      <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 md:gap-20 items-center relative z-10">
        ...
        <div style={revealStyle(isVisible, 0)} className="relative group">
          ...
          <div className="absolute bottom-4 right-4 w-20 h-20 gradient-gold rounded-sm flex items-center justify-center shadow-lg">
        ...
        <div>
          <p style={revealStyle(isVisible, 100)} className="text-gold text-sm tracking-widest uppercase mb-3">
            Quem é
          </p>
          <h2 style={revealStyle(isVisible, 150)} className="font-serif text-3xl md:text-5xl mb-8 text-navy">
            Dr. Jonathan Vidal
          </h2>
          ...
          {/* Pilares */}
          <div style={revealStyle(isVisible, 500)} className="flex gap-10 mt-12 pt-8 border-t border-gold/20">
            {["Harmonia", "Naturalidade", "Precisão"].map((item) => (
              <div key={item} className="text-center group">
                <div className="w-16 h-px gradient-gold mx-auto mb-3 group-hover:h-1 transition-all duration-300" />
